...
PIPE_OUTPUT = '2>&1 | tee experiment_scripts/dht_test/'

def start_visualization():
    cmd_str = "python3 ./DHT_logic/DHT_visualization.py -j my_apps_experiment.json -i commands.txt -d 300"
    subprocess.run(cmd_str, shell=True)

def run_demo_experiment():
    # cmd_str = "sudo mn --topo=single,8 --pre=./experiment_scripts/my_apps_experiment.sh"
    # subprocess.run(cmd_str, shell=True)
