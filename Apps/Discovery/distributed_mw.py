...
# ======================================== OTHER HELPERS ======================================== #
    ...
    def determine_topic_location(self, topic_hash, app_id, app_type):
        try:
            ...
            if ((self.predecessor.node_id < topic_hash < self.node_id) or
                ((self.predecessor.node_id > self.node_id) and
    ...
    def talk_to_neighbor(self, neighbor, new_node=None, which_neighbor=None, broker=None,
        ...
        try:
            ...
            if which_neighbor or start_node_id and not topic_hash and not all_pubs:
                return self.send_update_request(disc_req, neighbor_req, new_node, which_neighbor, broker, start_node_id)
            else: return self.send_locate_request(disc_req, neighbor_req, new_node, topic_info, topic_hash, start_node_id, all_pubs)
        except Exception as e: handle_exception(e)

    """send an update message to the given neighbor and wait for a response"""
    def send_update_request(self, disc_req, neighbor_req, new_node, which_neighbor, broker, start_node_id):
        try:
            ...
            if start_node_id: update_req.start_node_id = start_node_id
            if broker: 
    ...
    def send_locate_request(self, disc_req, neighbor_req, new_node, topic_info, topic_hash, start_node_id, all_pubs):
        try:
            ...
            else: disc_req.msg_type = discovery_pb2.LOCATE_NEW_NODE
