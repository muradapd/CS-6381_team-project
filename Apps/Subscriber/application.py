...
class SubscriberAppln():
  ...
  def driver(self):
    try:
      ...
      self.logger.debug(f"SubscriberAppln::driver - result: {str(result)}")
      ...
      while(not self.mw_obj.is_ready()): time.sleep(0.5)  # sleep between calls
      self.logger.info("All sub and pub apps registered and ready.")
      time.sleep(random.uniform(0.0, 1.0)) # sleep for a bit to not overwhelm system
      # Now, find the publishers that match our topics
      self.logger.info("Locating all publishers for our topics.")
      pubs = self.mw_obj.locate_pubs(self.topiclist)
      # Finally, subscribe and listen to the publishers
      self.logger.info("Subscribing to relevant publishers.")
