  ...
  def driver (self):
    ...
    try:
      ...
      if result == '1':
        ...
        result = 'failure'
      self.logger.debug (f"PublisherAppln::driver - result of registration: {result}")
      ...
      register_fname = f'PA2_DEMO/stats/{self.name}_{self.lookup}_register.csv'
      ...
      while (not self.mw_obj.is_ready ()):
        self.logger.info(f"PublisherAppln - Checking if system is ready...")
      ...
      is_ready_fname = f'PA2_DEMO/stats/{self.name}_{self.lookup}_isready.csv'
      ...
      self.logger.debug ("PublisherAppln::driver - finished publishing")
...
def parseCmdLineArgs ():
  ...
  parser.add_argument ("-n", "--name", required=True, default="pub", help="Some name assigned to us. Keep it unique per publisher")

  parser.add_argument ("-a", "--addr", required=True, default="localhost", help="IP addr of this publisher to advertise (default: localhost)")

  parser.add_argument ("-p", "--port", default="5577", help="Port number on which our underlying publisher ZMQ service runs, default=5577")
    
  parser.add_argument ("-d", "--discovery", required=True, default="localhost:5555", help="IP Addr:Port combo for the discovery service, default localhost:5555")

  parser.add_argument("-t", "--num_topics", type=int, choices=range(1,10), default=1, help="number of topics to select")

  parser.add_argument ("-c", "--config", default="PA2_DEMO/config.ini", help="configuration file (default: config.ini)")
